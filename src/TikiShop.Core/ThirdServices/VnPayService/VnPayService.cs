...
public class VnPayService : IVnPayService
{
    ...
    public async Task<ResultObject<string>> CreatePaymentUrl(int orderId, string clientIp)
    {
        ...
        if (order is null || order.Status != OrderStatus.Submitted)
        {
            ...
            return ResultObject<string>.Failed("Invalid order");
        }
        ...
        return ResultObject<string>.Success(paymentUrl);
    }

    public async Task<ResultObject<VnPayResponseDto>> ReturnUrlVnPay(ReturnUrlReq returnUrlParams)
    {
        ...
        if (!isValid)
        {
            ...
            return ResultObject<VnPayResponseDto>.Success(new VnPayResponseDto(VnPayResponseCode.InvalidSignature));
        }
        ...
        if (order is null)
        {
            ...
            return ResultObject<VnPayResponseDto>.Success(new VnPayResponseDto(VnPayResponseCode.OtherErrors));
        }

        if (Convert.ToInt32(order.Total) == returnUrlParams.vnp_Amount)
            return ResultObject<VnPayResponseDto>.Success(new VnPayResponseDto(VnPayResponseCode.PartialRefundOnly));

        if (order.OrderStatus == OrderStatus.AwaitingValidation)
            return ResultObject<VnPayResponseDto>.Success(new VnPayResponseDto(VnPayResponseCode.TransactionNotFound));
            ...
            transactionStatus == VnPayResponseCode.Success)
        {
            ...
            return ResultObject<VnPayResponseDto>.Success(new VnPayResponseDto(VnPayResponseCode.Success));
        }
