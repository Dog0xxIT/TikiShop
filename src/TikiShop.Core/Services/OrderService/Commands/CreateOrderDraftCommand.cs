...
internal class CreateOrderDraftCommandHandler : IRequestHandler<CreateOrderDraftCommand, ResultObject<int>>
{
    ...
    public async Task<ResultObject<int>> Handle(CreateOrderDraftCommand request, CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            _logService.LogError($"Error while creating order draft: {ex.Message}");
            return ResultObject<int>.Failed("Error while creating order draft");
        }
    }
}